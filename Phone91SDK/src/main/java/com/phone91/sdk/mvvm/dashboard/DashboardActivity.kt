...
//        getWindow().setFlags(WindowManager.LayoutParams.FLAG_SECURE, WindowManager.LayoutParams.FLAG_SECURE)
        ...
        if (intent.getStringExtra(WIDGETTOKEN) == null || intent.getStringExtra(WIDGETTOKEN).trim()
                .equals("")
        ) {
        ...
        if (intent.hasExtra(NAME) || intent.hasExtra(EMAIL) || intent.hasExtra(PHONE)) {
            if (intent.getStringExtra(NAME) == null || intent.getStringExtra(NAME).trim()
                    .equals("")
            ) {
                showShortToast("Please add name.")
                finish()
            }
            if (intent.getStringExtra(EMAIL) == null || intent.getStringExtra(EMAIL).trim()
                    .equals("")
            ) {
                showShortToast("Please add email.")
                finish()
            }
            if (intent.getStringExtra(PHONE) == null || intent.getStringExtra(PHONE).trim()
                    .equals("")
            ) {
...
//                getChannelList()
                ...
                if (paramListObject != null) {
                    ...
                    if (paramListObject?.default_params != null) {
                        ...
                        if (paramListObject?.default_params?.size!! > 0) {
                            Log.d(
                                "Hello",
                                "Team size is " + paramListObject?.default_params?.size.toString()
                            )
                        }
                    }
        ...
        dashboardActivityVM?.clientInfo?.observeForever {
            if (it != null) {
                clientObject = it
                name = clientObject?.data?.optString("param_1")
                email = clientObject?.data?.optString("param_3")
                phone = clientObject?.data?.optString("param_2")
...
//                    dashboardActivityVM.insertOnDB(channelObject)
                    ...
                    if (channelObject?.uuid != null) {
                        dashboardActivityVM?.insertWidgetDetail(
                            WidgetDBObject(
                                appDataManager?.getWidgetToken()!!,
...
//                     channelObject . team_id ="default_team"
    ...
    fun showChatWindow() {
        ...
        if (widgetObject?.teams != null && widgetObject?.teams?.size!! > 0) {
            ...
            navigationController.openTeamFragment(
                this,
                R.id.fl_dashboard_container,
                TeamFragment.newInstance(widgetObject!!)
            )
        } else {
...
//        showChooseUserFragmentS(category, subcategory, zipcode)
    ...
    public fun setClient(
        ...
        dashboardActivityVM?.setClient(channel, name, email, phone, uuid)
