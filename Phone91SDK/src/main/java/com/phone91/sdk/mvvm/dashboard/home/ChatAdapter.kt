...
class ChatAdapter(var context: Activity?, public var teamList: ArrayList<CallConnectionSignal>, var callbackClient:CallbackClient) :
...
//            holder.edName.setOnKeyListener(this)
            ...
            if (chatObject.sender == null) {
                ...
                if(chatObject.time!=null && !chatObject.time.toString().equals("-1"))
                    holder.txMeTime.text=TimeUtil.covertTimeToText(chatObject.time)//.toDuration(chatObject.time)
                else
                ...
                if (chatObject.attachment_url != null && !chatObject.attachment_url.equals("") ) {
                    ...
                    if(chatObject.mime_type.equals("image/*"))
                       Glide.with(context!!)
                        .load(chatObject.attachment_url)
                        .apply(requestOptions)
                        .into(holder.imgMe)
                    else
                        holder.imgMe.setImageDrawable(ContextCompat.getDrawable(context!!, R.drawable.txt));
                ...
                holder.txtYouTime.text=TimeUtil.covertTimeToText(chatObject.time)//TimeUtil.toDuration(chatObject.time)
                if (chatObject.content == null)
                    holder.txtYou.visibility = View.GONE
                else {
                ...
                if (chatObject.attachment_url != null && !chatObject.attachment_url.equals("")) {
                    ...
                    if(chatObject.mime_type?.contains("image/")==true)
                        Glide.with(context!!)
                            .load(chatObject.attachment_url)
                            .apply(requestOptions)
                            .into(holder.imgYou)
                    else
                        holder.imgYou.setImageDrawable(ContextCompat.getDrawable(context!!, R.drawable.txt));
                } else
                    holder.imgYou.visibility = View.GONE
            }
        }
    ...
    inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view), View.OnClickListener{
        ...
        public fun bind(){
            ...
            edEmail.setOnKeyListener(View.OnKeyListener {  v, keyCode, event ->
                if (event.action === KeyEvent.ACTION_DOWN && keyCode == KeyEvent.KEYCODE_ENTER) {
                    if(edEmail.text.toString().trim().equals("") || !Patterns.EMAIL_ADDRESS.matcher(edEmail.text.toString().trim()).matches())
                    {
            ...
            edPhone.setOnKeyListener(View.OnKeyListener {  v, keyCode, event ->
                if (event.action === KeyEvent.ACTION_DOWN && keyCode == KeyEvent.KEYCODE_ENTER) {
                    ...
                    if(edPhone.text.toString().trim().equals("") || !Patterns.PHONE.matcher(edPhone.text.toString().trim()).matches())
                    {
                        Toast.makeText(context, "Please enter name", Toast.LENGTH_SHORT)
                            .show()
                        return@OnKeyListener true
        ...
        override fun onClick(v: View?) {
            when (v?.id){
                ...
                R.id.btnPhone->
                    ...
                    }else if(edEmail.text.toString().trim().equals("") || !Patterns.EMAIL_ADDRESS.matcher(edEmail.text.toString().trim()).matches())
                         {
                    Toast.makeText(context, "Please enter valid email", Toast.LENGTH_SHORT)
                        .show()
